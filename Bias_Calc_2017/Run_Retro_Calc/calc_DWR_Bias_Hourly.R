...
# Get basin number passed in. We will subset all info based on this basin since
# this is spread out amongst different processors.
args = commandArgs(trailingOnly=TRUE)
basinNum <- as.integer(c(args[1]))
...
biasStatsDir <- '/glade/u/home/karsten/RHAP_home/wrf_hydro_analysis/URG_2017/Bias_Calc_2017/Run_Retro_Calc/BIAS_STATS_HOURLY'
plotDir <- '/glade/u/home/karsten/RHAP_home/wrf_hydro_analysis/URG_2017/Bias_Calc_2017/Run_Retro_Calc/PLOTS_HOURLY'
obsDir <- '/glade/u/home/karsten/RHAP_home/wrf_hydro_analysis/URG_2017/Bias_Calc_2017/Run_Retro_Calc/OBS_HOURLY'
modDir <- '/glade/u/home/karsten/RHAP_home/wrf_hydro_analysis/URG_2017/Bias_Calc_2017/Run_Retro_Calc/MODEL_DATA'

metaData <- read.table(ptsCsv,header=TRUE,sep=',',stringsAsFactors = FALSE)
metaData <- metaData[basinNum,]
dwrIds <- metaData$DWR_ID
numPts = length(metaData$ID)

# Open RouteLink file, loop through and calculate index values for each point.
indexValues = array(0,dim=c(numPts))
...
for (pt in 1:numPts){
	indexValues[pt] <- which(links == metaData$ID[pt])
}
...
#obsStrData <- GetCoDwrData(dwrIds,paramCodes=c("DISCHRG"),timeInt="hourly",startDate="10/01/2003",endDate="10/01/2016")

load(paste0(obsDir,'/Obs_',dwrIds[1],'.Rdata'))
load(paste0(modDir,'/modStr_',dwrIds[1],'.Rdata'))
...
#modData <- as.data.table(plyr::ldply(filesList, ReadChFile, indexValues, dwrIds, .parallel = FALSE))

#print('SAVING OUTPUT')
#save(obsStrData,file=paste0('Obs_',dwrIds[1],'.Rdata'))
#save(modData,file=paste0('modStr_',dwrIds[1],'.Rdata'))

modData <- as.data.frame(modData)

#modDataParent <- modData
#obsStrDataParent <- obsStrData
...
# Loop through each DWR station and calculate biases, etc
# Create dataframe to hold data
dfOut1 <- data.frame(matrix(NA,ncol=6,nrow=(nDays*24*length(years))))
names(dfOut1) <- c('Observed_cfs','Observed_kaf','Modeled_cfs','Modeled_kaf','POSIXct','Basin')
dfOut1$POSIXct <- as.POSIXct('1900-01-01',"%Y-%m-%d")

dfOut2 <- data.frame(matrix(NA,ncol=10,nrow=(nDays*24)))
...
dfOut3 <- data.frame(matrix(NA,ncol=7,nrow=(nDays*24*2)))
names(dfOut3) <- c('Mean_cfs','Median_cfs','Mean_kaf','Median_kaf','POSIXct','Basin','Tag')
dfOut3$POSIXct <- as.POSIXct('1900-01-01',"%Y-%m-%d")

dfOut4 <- data.frame(matrix(NA,ncol=4,nrow=(nDays*24*length(years)*2 ) ) )
names(dfOut4) <- c('POSIXct','Basin','q_cfs','Tag')
dfOut4$POSIXct <- as.POSIXct('1900-01-01',"%Y-%m-%d")

# Make an ouptut dataframe that we will ouptut everytime we loop through a basin.
dfBias <- data.frame(matrix(NA,ncol=11,nrow=1))
names(dfBias) <- c('Basin','mean_obs_kaf','mean_mod_kaf','median_obs_kaf',
                   'median_mod_kaf','mean_mod_bias','median_mod_bias',
                   'mean_bias_factor','median_bias_factor','percentObsAvail',
                   'numYearsUsed')
...
for (basin in dwrIds){
   ...
   print(paste0('PROCESSING BASIN: ',basin))

   # Create array to hold quality-control information on what percentage of year contained observations.
   yearsObsPercent <- array(0.0,c(length(years)))
   yearsUsed <- array(0,c(length(years)))

   obsStrData <- subset(obsStrData,Station == basin)
   modData <- subset(modData,DWR_ID == basin)
   for (year in years){
      #print(year)
      wy <- year + 1
      ...
      for (day in 1:nDays){
         for (hour in 1:24){
            ...
            ind <- which(obsStrData$POSIXct == dCurrent & obsStrData$Station == basin)
	    ...
	    if (accRunoffObs != 0.0){
            ...
            ind <- which(modData$POSIXct == dCurrent & modData$DWR_ID == basin)
      ...
      # Do some QC work here. Determine the fraction of the year observations were available. If we are less than 75%, 
      # Throw out accumulated variables and set to NA here. 
      indYear <- which(strftime(dfOut1$POSIXct,'%Y') == as.character(year) & dfOut1$Basin == basin)
      yearsObsPercent[yearCount] <- (length(which(!is.na(dfOut1$Observed_cfs[indYear])))/length(dfOut1$Observed_cfs[indYear]))*100.0
      if (yearsObsPercent[yearCount] >= 75.0){
         yearsUsed[yearCount] <- 1
      } else {
         # Set all accumulated flow for this year to NA so they don't go into mean calculations
         dfOut1$Observed_kaf[indYear] <- NA
         dfOut1$Modeled_kaf[indYear] <- NA
      }
   ...
   for (day in 1:nDays){
      for (hour in 1:24){
         ...
         dateStr <- strftime(dCurrent,'%m-%d:%H')

         ind <- which(dfOut1$Basin == basin & strftime(dfOut1$POSIXct,'%m-%d:%H') == dateStr)
   ...
   path1 <- paste0(plotDir,'/mean_acc_runoff_kaf_',basin,'.png')
   path2 <- paste0(plotDir,'/median_acc_runoff_kaf_',basin,'.png')

   dfTmp <- subset(dfOut3,Basin == basin)
   dfTmp1 <- subset(dfOut2,Basin == basin)

   # If insufficient observations were present to calculate biases, bypass creating the accumulated
   # flow plots as there will be nothing to plot.
   if (length(which(yearsUsed == 1)) != 0) {
  ...
  # Save stats to an Rdata file
  dfBias$Basin <- basin
  dfBias$mean_obs_kaf <- meanObsKaf
  dfBias$mean_mod_kaf <- meanModKaf
  dfBias$median_obs_kaf <- medianObsKaf
  dfBias$median_mod_kaf <- medianModKaf
  dfBias$mean_mod_bias <- meanBias
  dfBias$median_mod_bias <- medianBias
  dfBias$mean_bias_factor <- meanFactor
  dfBias$median_bias_factor <- medianFactor
  dfBias$percentObsAvail <- sum(yearsObsPercent)/(length(yearsUsed)*100.0)
  dfBias$numYearsUsed <- length(which(yearsUsed == 1))

  # If no years were used, we will default the bias-correction factor to 1.0, which
  # means no bias correction will take place.
  if (dfBias$numYearsUsed == 0) {
     dfBias$mean_bias_factor <- 1.0
     dfBias$median_bias_factor <- 1.0
  }

  outPath <- paste0(biasStatsDir,'/',basin,'_biasStats.Rdata')
  save(dfBias,file=outPath)
